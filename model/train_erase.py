...
def lin_to_srgb(img):
    image_small = img <= 0.0031308
    image_small = img * image_small.astype(int)
    image_small *= 12.92

    image_big = img > 0.0031308
    image_big = img * image_big.astype(int)
    image_big = np.power(image_big, 1/2.4)
    image_big *= 1.055
    image_big -= 0.055
    linear_image = image_big + image_small
    return linear_image
...
def main():
    ...
    G = Generator(cfg, in_channels=3, mode='erase').to(device)
        ...
        if (step + 1) % cfg.gen_example_interval == 0:
            ...
            with torch.no_grad():
                for inp in eval_loader:
                    ...
                    #o_mask_s, o_b = lin_to_srgb(o_mask_s), lin_to_srgb(o_b)
