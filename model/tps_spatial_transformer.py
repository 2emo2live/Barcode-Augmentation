...
class TPSSpatialTransformer(nn.Module):
    ...
    def _interpolate(self, im, x, y):
        ...
        zero = torch.tensor(0, dtype=torch.int32).to(device)
        max_y = torch.tensor(height - 1, dtype=torch.int32).to(device)
        max_x = torch.tensor(width - 1, dtype=torch.int32).to(device)
        ...
        base = self._repeat(torch.arange(0, num_batch) * dim1, out_height * out_width).to(device)
    ...
    def solve_system(self, target_ctrl_points):
        ...
        vec = vec.reshape(num_batch, num_point, 2).to(device)
        p = torch.cat([torch.ones([num_batch, num_point, 1]).to(device), coord], 2)  # [bn, pn, 3]
        ...
        W_1 = torch.cat([torch.zeros([num_batch, 3, 3]).to(device), torch.transpose(p, 2, 1)], 2)  # [bn, 3, pn+3]
    ...
    def _meshgrid(self, height, width, coord):
        ...
        grid = torch.cat((torch.ones(x_t_flat_g.shape).to(device), x_t_flat_g, y_t_flat_g, r), 1)
        return grid
