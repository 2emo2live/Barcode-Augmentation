...
def calculate_corners(H, center, x, y, tx, ty):
    points = np.ones((3, 4), dtype=np.float32)
    points[:2, 0] = np.array([tx, ty], dtype=np.float32).T
    points[:2, 1] = np.array([tx + x, ty], dtype=np.float32).T
    points[:2, 2] = np.array([tx + x, ty + y], dtype=np.float32).T
    points[:2, 3] = np.array([tx, ty + y], dtype=np.float32).T

    perspected_points = center_pointsPerspective(points, H, center)
    perspected_points[0, :] /= perspected_points[2, :]
    perspected_points[1, :] /= perspected_points[2, :]

    return perspected_points[:2, :]
...
def perspective(img, rotate_angle, zoom, shear_angle, perspect, pad): # w first
    ...
    border = calculate_corners(H, canvas_center, img_h, img_w, tlx, tly)
    ...
    return resimg, border.astype(int)
